          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="lg:col-span-2 bg-card rounded-xl border">
              <div className="p-4 border-b flex items-center justify-between">
                ...
                <div className="text-xs text-muted-foreground">{waitlist.length} signed up</div>
              </div>
              {waitlist.length === 0 ? (
                <div className="p-4 text-sm text-muted-foreground">No one on the waitlist yet.</div>
              ) : (
                    ...
                    <tbody className="divide-y">
                      {(waitlist as any[]).map((w) => (
                        <tr key={w.id}>
                          ...
                          <td className="px-4 py-2">{w.ticket_tiers?.name ?? "—"}</td>
                          <td className="px-4 py-2 text-right">{w.quantity}</td>
