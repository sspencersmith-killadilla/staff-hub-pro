...
function ManagePage() {
    ...
    return (
      <div className="mx-auto max-w-xl px-6 py-12 text-center">
        <p className="text-slate-700">
          You don't have a community organization yet.
        </p>
        <Link
          to="/community/apply"
      ...
      <div className="mx-auto max-w-xl px-6 py-12 text-center">
        <span
          className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${
            STATUS_STYLES[org.status] ?? "bg-slate-100"
          }`}
        >
          {org.status}
        </span>
        <h1 className="mt-3 text-2xl font-bold text-slate-900">{org.name}</h1>
        <p className="mt-2 text-sm text-slate-600">
          Your organization isn't approved yet, so you can't add locations or
          submit events. We'll email{" "}
          <strong>{org.contact_email}</strong> once a decision is made.
        </p>
        {org.staff_notes && (
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      ...
      <p className="mt-1 text-sm text-slate-600">Manage your venues and events.</p>
      ...
      <div className="mt-6">
        {tab === "events" ? <EventsTab /> : <LocationsTab />}
      </div>
    </div>
  );
}
...
function LocationsTab() {
  ...
  const { data: locs, isLoading } = useQuery({
    queryKey: ["community", "my-locations"],
    queryFn: () => fetchLocs(),
  });
  ...
  function invalidate() {
    qc.invalidateQueries({ queryKey: ["community", "my-locations"] });
  }
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">
          No locations yet. Add the venues where you host events.
        </div>
      ) : (
...
function EventsTab() {
  ...
  function invalidate() {
    qc.invalidateQueries({ queryKey: ["community", "my-events"] });
